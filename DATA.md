...
Useful for anyone doing similar BESS-arb research. Lists what's free,
what needs a key, and what's genuinely paywalled in this domain.

### Sources requiring a free key (one-time email signup)
...
### Sources with no key required (anonymous HTTPS / S3)

| Source                       | Access                                                |
|------------------------------|-------------------------------------------------------|
| NOAA HRRR forecasts          | `s3://noaa-hrrr-bdp-pds` anonymous                    |
| NOAA GFS forecasts           | `s3://noaa-gfs-bdp-pds` anonymous                     |
...
| ERCOT MIS historical CSVs    | direct HTTPS                                          |
| ERCOT historical load ZIPs   | https://www.ercot.com/gridinfo/load/load_hist         |

For anonymous S3, use `aws s3 cp --no-sign-request` or `boto3` with
`config=botocore.UNSIGNED`. No AWS account required.

### Relevant ERCOT Public API report IDs

| Report ID    | Description                                          | Used here? |
|--------------|------------------------------------------------------|------------|
| NP6-785-ER   | Settlement Point Prices (RTM SPP)                    | yes (via `gridstatus`) |
| NP4-732-CD   | Wind Power Production — actual + STWPF + WGRPP       | yes        |
| NP4-737-CD   | Solar Power Production — actual + STPPF              | yes        |
| NP3-560-CD   | Seven-Day Load Forecast by Model and Weather Zone    | no         |
| NP3-233-CD   | Hourly Resource Outage Capacity                      | yes (post-hoc, see FINDINGS) |
| NP4-742-CD   | Hourly Aggregated Wind Output                        | no         |
| NP1-346-ER   | Unplanned Resource Outages (forced + maintenance)    | no         |

The ERCOT Public API archive holds ≥ 7 years of history per report.
Each document has a `postDatetime` (publish timestamp) → vintaged
"as-of" semantics out of the box. Authentication is OAuth2 ROPC against
ERCOT's B2C tenant; see `src/data/ercot_api.py` for the flow.

### What we genuinely couldn't get cheaply

- **Live, sub-hourly operator decisions** — commitment / dispatch signals
  an ERCOT market participant sees internally. Requires paid market-
  participant credentials.
- **Individual generator bids** — offers and commitment prices by unit.
  Some publicly released with 60-day lag, attribution obfuscated.
- **Commercial blended weather forecasts** — DTN, Atmospheric G2,
  Radiant Solutions, etc. NOAA NWP isn't the same as a private
  ensemble.
- **Intraday ERCOT-internal forecast revisions** — the SCED / RAS
  intermediate forecasts. Partially in NP4-442-CD but not all models.

For the experiment-by-experiment "what we tried, what was rejected,
what we missed" outcomes, see [FINDINGS.md](FINDINGS.md).
